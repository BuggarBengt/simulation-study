...
data(jobs) #let's use this our "true" scenario
...
colnames(to.plot) = c("interaction.coefficient", "est.nde", "est.nie", "nde.bias", "nie.bias", "nde.percent.bias", "nie.percent.bias","nde.coverage", "nie.coverage")
...
for (i in 1:length(result)) { # run simulations while increasing interaction effect
  ...
  to.plot[i, 4] = mean(result[[i]]$est.nde) - mean(result[[i]]$true.nde)
  to.plot[i, 5] = mean(result[[i]]$est.nie) - result[[i]]$true.nie
  to.plot[i, 6] = (mean(result[[i]]$est.nde) - mean(result[[i]]$true.nde))/mean(result[[i]]$true.nde)
  to.plot[i, 7] = (mean(result[[i]]$est.nie) - result[[i]]$true.nie)/result[[i]]$true.nie
  to.plot[i, 8] = mean(ifelse(result[[i]]$true.nde < result[[i]]$CI.nde.upper & result[[i]]$true.nde > result[[i]]$CI.nde.lower, 1, 0))
  to.plot[i, 9] = mean(ifelse(result[[i]]$true.nie < result[[i]]$CI.nie.upper & result[[i]]$true.nie > result[[i]]$CI.nie.lower, 1, 0))
}
