...
Total effect is unbiased:

```{r echo=F}
int.total = ggplot() +
  ggtitle("Total effect(TE) (misspecified model)") +
  theme(plot.title = element_text(size = 9, face = "bold"), legend.position = "bottom") +
  geom_line(data = data.frame(to.plot), aes(x=interaction.coefficient, y = true.nde+true.nie, col = "True TE")) +
  geom_line(data = data.frame(to.plot), aes(x=interaction.coefficient, y = est.nde+est.nie, col = "Estimated TE")) +
  xlab('interaction.coef') +
  ylab('total effect')
...
nde1 = ggplot() + 
  geom_line(data = data.frame(nde.est = rep(to.plot[round(to.plot[, "interaction.coefficient"], 2)==-0.20, "est.nde"], length(rhos))), aes(x=rhos, y = nde.est, col = "misspec.NDE")) +
  ...
  xlab('correlation') +
  ylab('effect') +
  ggtitle("ZM interaction = -0.2") +
  theme(legend.position = "none")
nde2 = ggplot() + 
  geom_line(data = data.frame(nde.est = rep(to.plot[round(to.plot[, "interaction.coefficient"], 2)==0, "est.nde"], length(rhos))), aes(x=rhos, y = nde.est, col = "misspec.NDE")) +
  ...
  xlab('correlation') +
  ylab('effect') +
  ggtitle("ZM interaction = 0") 
nde3 = ggplot() + 
  geom_line(data = data.frame(nde.est = rep(to.plot[round(to.plot[, "interaction.coefficient"], 2)==0.2, "est.nde"], length(rhos))), aes(x=rhos, y = nde.est, col = "misspec.NDE")) +
  ...
  theme(legend.position = "none")
nde4 = ggplot() + 
  geom_line(data = data.frame(nde.est = rep(to.plot[round(to.plot[, "interaction.coefficient"], 2)==0.5, "est.nde"], length(rhos))), aes(x=rhos, y = nde.est, col = "misspec.NDE")) +
